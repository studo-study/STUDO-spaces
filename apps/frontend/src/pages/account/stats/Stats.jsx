export default function Stats({ title, waarde, svg }) {
  return (
    <div className="flex bg-studowhite border-transparent border-studoborder
      ...
      <img src={svg} className="h-4 sm:h-5 flex-shrink-0 dark:brightness-0 dark:invert" alt="svg" />
      <span
        className="text-sm sm:text-base md:text-lg font-bold text-studodarkblue dark:text-white truncate">{title}:</span>
      <span className="text-sm sm:text-base md:text-lg text-studodarkblue dark:text-white">{waarde}</span>
    </div>
  );
}