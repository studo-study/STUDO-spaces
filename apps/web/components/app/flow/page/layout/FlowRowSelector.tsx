...
const FlowRowSelector = (props: FlowRowSelectorProps) => {
    ...
    return <div className={`${isActive && "hidden"} absolute bottom-20 gap-1 w-full h-10 flex flex-row items-center overflow-hidden rounded-full border border-studoborder/30 bg-studogrey/30 `}>
        <div className={"w-fit h-full"}>
            <Link href={"/flow/" + id + "/overview"} className={`w-fit min-h-full justify-center h-full min-w-24 flex dark:text-white text-studodarkblue items-center hover:bg-studogrey/30 border-r border-r-studoborder/30 transition-all duration-300 px-3 ${isActive && "font-bold bg-studogrey/30"}`}>
