...
export default function SettingsTrigger() {
  ...
  return (
    <div className={"w-full h-fit relative"}>
      <div
        className={
          "relative w-full h-20 flex items-center justify-end flex-row"
        }
      >
        <div
          ref={containerRef}
          onClick={toggleOpen}
          className={
            "min-w-12 h-12 dark:text-white cursor-pointer border border-transparent transition-all rounded-full shadow-xl bg-studogrey/30 flex  items-center justify-center hover:border-studogrey"
          }
        >
          <IoIosSettings
            size={25}
            className={"active:rotate-180 duration-300 transition-all"}
          />
        </div>
      </div>
      <SettingsPopup
