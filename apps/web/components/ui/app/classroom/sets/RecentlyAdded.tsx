...
export default function RecentlyAdded({items}: ListItemProps) {
    ...
    const thisWeek = filteredSets.filter(item =>
        now - new Date(item.created_at).getTime() <= ONE_WEEK_IN_MS
    );
    ...
    const rest = filteredSets.filter(item =>
        now - new Date(item.created_at).getTime() > ONE_WEEK_IN_MS * 2
    );
    const pinned = []
    return (<>
            ...
            <div className={"w-full min-h-full h-full flex flex-col gap-3"}>
                {pinned.length > 0 && (<><span
                    className={"font-bold text-studodarkblue dark:text-white"}>{t("pinned_sets")}:</span>
                    <div className={"w-full h-1/3 flex flex-col gap-5"}>
                        {pinned.length === 0 && <span
                            className={"w-full h-full text-studodarkblue dark:text-white flex items-center justify-center"}>{t("no_pinned")}</span>}
                    </div>
                </>)}
                <div className={"w-full h-2/3 flex flex-col gap-5"}>
                    ...
                    {thisWeek.length ?
                        (<div className={"w-full flex flex-col gap-10"}>
                            <div className={"w-full h-fit flex flex-col gap-5"}>
                                <span className={"dark:text-white text-sm text-studodarkblue font-bold"}>{t("this_week")}</span>
                                {thisWeek.map((item, index) => (<ListItem set={item} key={index} t={t}/>))}
                            </div>
                            {lastWeek.length != 0 && (<div className={"w-full h-fit flex flex-col gap-5"}>
                                ...
                                {lastWeek.map((item, index) => (<ListItem set={item} key={index} t={t}/>))}
                            </div>)}
                            {rest.length != 0 && (<div className={"w-full h-fit flex flex-col gap-5"}>
                                <span className={"dark:text-white text-sm text-studodarkblue"}>{t("other")}</span>
                                {rest.map((item, index) => (<ListItem set={item} key={index} t={t}/>))}
                            </div>)}
                        </div>)
                        : (
                    ...
                    {items.sets.length == 0 && <span
                        className={"w-full h-full text-studodarkblue dark:text-white flex items-center justify-center"}>{t("no_sets")}</span>}
                </div>
    ...
    return (
        ...
        >
            ...
            <div className={`flex flex-row gap-3 items-center "w-full sm:w-1/2 sm:flex-1`}>
                <div className={"w-15 h-15 rounded-2xl bg-studogrey/30 flex items-center justify-center"}>
                ...
                <div className={"w-fit flex flex-col pt-1"}>
                    <div className={"w-fit flex flex-row gap-3"}>
                        <Image src={iconSrc} width={0} height={0}
                    ...
                    <span
                        className={"w-fit text-studodarkblue/30 dark:text-white/30 text-sm"}>{t("added_by")} {set.added_by}</span>
                </div>
