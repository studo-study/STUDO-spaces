    ...
    return (<>
        ...
        <div className="w-full h-full flex flex-col">
            ...
            <div className="relative w-full flex-1 min-h-0 overflow-y-auto scroll-hidden flex flex-col gap-5 pb-30">
                ...
                <div
                    className="fixed z-10 bottom-15 h-25 min-w-1/2 bg-linear-0 dark:from-bg-dark from-bg-white to-transparent"/>
            </div>
        </div>
    </>)
}
...
function JoinRequestItem({t, request, joinRequests, setJoinRequests}: JoinRequestProps) {
    const toggleRemove = () => {
    ...
    >
        <div className={"w-fit flex flex-row gap-5 items-center"}>
            ...
            <Link href={"/profile/" + request.id}
                  className={"dark:text-white hover:underline text-studodarkblue"}> {request.displayName} {" "} {t("w_l_t_j")}</Link>
        </div>
        <div className={"w-fit flex flex-row items-center gap-5 dark:text-white text-studodarkblue"}>
            <button onClick={toggleRemove}
                    className={"cursor-pointer hover:bg-emerald-400 active:scale-95 transition-all duration-300 w-10 h-10 rounded-full bg-studogrey items-center justify-center flex"}>
                <FaCheck size={18}/></button>
            <button onClick={toggleRemove}
                    className={"cursor-pointer hover:bg-rose-400  active:scale-95 transition-all duration-300 w-10 h-10 rounded-full bg-studogrey items-center justify-center flex"}>
...
function UserItem({t, user}: UserItemProps) {
    return (<Link href={"/profile/" + user.user_id} className={`max-h-15 min-h-15 flex-col sm:flex-row px-3 py-4 sm:py-0 max-w-full min-w-full cursor-pointer
        ...
        <div className={"flex items-center gap-3 min-w-fit"}>
            {user.role === "teacher" ? <span
                    className={"w-fit text-studodarkblue/30 border-2 border-studoborder bg-studogrey/20 px-3 py-1 rounded-full dark:text-white/30"}>{user.role}</span>
                : <span className={"text-studodarkblue/30 dark:text-white/30 text-sm pr-3"}>
