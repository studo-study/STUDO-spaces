...
export default function Tracks({ tracks }: TracksProps) {
  ...
  return (
    <div className={"w-full h-fit flex flex-col gap-5"}>
      {filteredTracks.map((group, index) => (
        <TrackContainer key={index} index={index} group={group} />
      ))}
    </div>
  );
}
...
function TrackContainer({ group }: TrackContainerProps) {
  ...
  return (
    <div className={"w-full h-fit flex flex-col gap-5 pl-5"}>
      <div
        onClick={toggleOpen}
        className={
          "w-full h-fit flex cursor-pointer flex-row justify-between items-center gap-3 pr-3"
        }
      >
