...
// Metadata apart — dit verandert niet, dus geen state nodig
    const columnMeta: Record<string, { title: string; icon: React.ReactNode; description: string }> = {
        to_do: {
            title: "To Do",
    ...
    //use effects
    const handleMove = useCallback((itemId: string, fromStatus: string, toStatus: string, newIndex: number) => {
        setColumns(prev => {
            const updated = {...prev};
    ...
    return (<div className={"w-full h-full flex flex-col gap-5 dark:text-white text-studodarkblue"}>
        <div className={"w-full h-fit flex flex-row items-center justify-between"}>
            ...
            <div className={"w-fit h-fit  flex flex-row items-center justify-between gap-3 text-sm font-bold"}>
                <span
                    className={"w-fit px-3 py-1 cursor-pointer rounded-full border border-studoborder/30 bg-studogrey/30"}>My Issues</span>
        ...
        <div ref={itemContainerRef} className="w-full h-full grid grid-cols-3 gap-7">
