...
import GridHeader from "@/components/ui/app/admin/reports/GridHeader";
import IssueItem from "@/components/ui/app/admin/reports/Issue";
...
interface ReportColumnProps {
    ...
    items: Issue[];
    status: string;
    onMove: (itemId: string, fromStatus: string, toStatus: string, newIndex: number) => void;
}

export default function ReportColumn({
                                         title, icon, count, description, items, status, onMove
                                     }: ReportColumnProps) {
