  ...
  return (
    ...
    >
      <div className={"flex flex-row gap-3"}>
        <div
          className={`bg-${color}-400/20 text-${color}-500 min-w-8 min-h-8 w-8 h-8 rounded-lg flex items-center justify-center`}
        >
          <Icon size={20} />
        </div>
        <span
      ...
      <span className={"text-xs dark:text-white text-studodarkblue opacity-50"}>
        {board.courses} {board.courses === 1 ? t("course") : t("courses")}
      </span>
    </Link>
  );
};
