...
const CourseContainer = (props: CoursesProps) => {
    ...
    return (<section className="flex flex-col gap-5 overflow-visible">
        <SectionHeader sectionIcon={<PiBooks />} title={t("courses_overview_title")} linkText={t('all_courses')} href={'/your-files/courses'}/>
        <Container height={"30"}>
            <div className="min-w-full w-full flex flex-row gap-5">
                {items.map((item, index) => (<CourseItem course={item} key={index}/>))}
