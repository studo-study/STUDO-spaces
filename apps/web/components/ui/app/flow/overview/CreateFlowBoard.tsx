...
const CreateFlowBoard = (props: CreateFlowBoardProps) => {
  ...
  //voorgekauwde metadata
  const date = new Date();
  const jaar_1 = date.getFullYear().toString();
  const jaar_2 = (date.getFullYear() + 1).toString();
  const sem = date.getMonth() + 1 >= 9 ? "semester 1" : "semester 2";
  ...
  const [startYear, setStartYear] = useState<string>(jaar_1);
  const [endYear, setEndYear] = useState<string>(jaar_2);
  const [semester, setSemester] = useState<string>(sem);
  ...
  return (
    <PopupBackdrop isOpen={createOpen} setIsOpen={setCreateOpen}>
      ...
      >
        <span
          className={
            "text-xs px-3 py-1 border border-studoborder/30 rounded-xl bg-studogrey/50 absolute top-2 left-2 font-bold text-studodarkblue dark:text-white"
          }
        >
          {t("create_title")}
        </span>
