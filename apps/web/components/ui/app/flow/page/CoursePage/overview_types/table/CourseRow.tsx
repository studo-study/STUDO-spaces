...
import RowSelector from "@/components/ui/app/flow/page/CoursePage/overview_types/table/RowSelector";
...
const CourseRow = (props: CourseRowProps) => {
    ...
    const onChange = (updates: Record<string, unknown>) => updateRow(rowId, updates);
    ...
    return (<div className={`flex flex-col border-b border-studoborder/30 cursor-pointer`}>
            <div style={{gridTemplateColumns: containsRes ? "3% 12% 49% 20% 12% 4%" : "3% 12% 69% 12% 4%"}}
                ...
                <div className={"px-2 h-full overflow-visible w-full truncate flex items-center"}>
                    <RowSelector
                        ...
                        options={[
                            ...
                            {icon: <IoDocumentTextOutline className={"text-blue-500"}/>, value: "notes", label: "Notes"},
                            ...
                            {icon: <RiPencilFill className={"text-blue-500"}/>, value: "sample_exam", label: "Sample Exam"},
                ...
                <div onClick={toggleOpen} className={"h-full flex items-center justify-center"}>{isOpen ?
                    <IoChevronUpOutline/> :
                    <IoChevronDownOutline/>}</div>
            </div>
            {isOpen && <div style={{gridTemplateColumns: "3% 97%"}}
                            className={"w-full h-fit grid"}>
                <div onClick={toggleOpen} className={"h-full flex items-center justify-center group py-2 pb-4"}>
                    <div className={"h-full w-0.5 border group-hover:border-white border-studoborder/30 rounded-full"}/>
                </div>

                <div
                    className={"flex flex-row items-center justify-between gap-3 px-2 py-3 border-t border-studoborder/30 w-full"}>
                    ...
                    <div className={"w-1/3 flex items-center justify-baseline"}>
                        <RowSelector
                            label={t("priority")}
                            options={[
                                {value: "no_priority", label: "No priority", dot: "bg-studogrey/50"},
...
function StatusSelector({status, setStatus}: StatusIconProps) {
    const t = useTranslations("flow.course.row")
    const options = [
        {icon: <TbCircle className={"text-rose-500"}/>, value: "not_started", label: t("not_started")},
        {icon: <RiProgress4Line className={"text-amber-400"}/>, value: "doing", label: t("doing")},
        {icon: <RiProgress8Line className={"text-emerald-500"}/>, value: "done", label: t("done")},
    ]
