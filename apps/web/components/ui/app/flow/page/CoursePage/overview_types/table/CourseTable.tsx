...
import {useFlowStore, selectCourseTotals} from "@/store/slices/flow/flowStore";
import {useShallow} from "zustand/react/shallow";

const CourseTable = () => {
    const rows = useFlowStore((s) => s.activeCourse?.rows) ?? [];
    const {totalLength, done, inProgress, containsRes} = useFlowStore(useShallow(selectCourseTotals));
    ...
    return (<div className={"w-full flex flex-col gap-7"}>
        ...
        <div>
            ...
            <div>
                {rows.map((row) => <CourseRow key={row.id} rowId={row.id} containsRes={containsRes} />)}
            </div>
