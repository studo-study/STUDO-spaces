...
export default function GridHeader({
  ...
  return (
    <div
      className={
        "w-full h-fit flex flex-col gap-2 dark:text-white text-studodarkblue"
      }
    >
      <div className={"w-full h-fit flex flex-row gap-3 justify-between "}>
        <div className={"w-fit h-fit flex flex-row items-center gap-2 "}>
          ...
          <div className={"font-bold flex flex-row items-center gap-1"}>
            <span>{title}</span>
            <span className={"bg-studogrey/30 text-sm rounded-3xl px-2 py-0.5"}>
              {count}
            </span>
          </div>
        </div>
        <div
          className={"w-fit flex flex-row gap-3 justify-center items-center"}
        >
