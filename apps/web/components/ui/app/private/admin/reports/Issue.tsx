...
import IssueDetails from "@/components/ui/app/private/admin/reports/IssueDetails";
...
export default function IssueItem({ item, status }: IssueProps) {
  ...
  return (
    ...
    >
      <div
        className={
          "w-full min-h-fit flex text-white flex-row gap-3 justify-between "
        }
      >
        <span className={"w-fit h-fit text-sm text-zinc-400"}>
          #{item?.number}
        </span>
        <div>
          {item.assigneeId ? (
            ...
            <div
              className={
                "min-w-5 min-h-5 w-5 h-5 cursor-pointer max-w-5 max-h-5 rounded-full bg-zinc-800/30 flex items-center justify-center opacity-50"
              }
            >
              <LuCircleUserRound />
            </div>
      ...
      <div className={"w-full flex flex-row gap-2 items-center"}>
        <div
          className={
            "max-w-6 max-h-6 h-6 w-6 rounded-xl bg-studogrey flex items-center justify-center opacity-50"
          }
        >
...
function getPriority(priority: string) {
  switch (priority) {
    case "low":
      return <PiCellSignalLowFill size={12} />;
    case "medium":
      return <PiCellSignalMediumFill size={12} />;
    case "high":
      return <PiCellSignalHighFill size={12} />;
    case "urgent":
      return <CgDanger size={12} />;
  }
...
function getProgress(progress: string) {
  switch (progress) {
    case "to_do":
      return <MdOutlineBrightness1 size={14} className={"text-rose-500"} />;
    case "in_progress":
      return <RiProgress4Line className={"text-amber-400"} />;
    case "done":
      return <RiProgress8Line className={"text-emerald-400"} />;
  }
}
