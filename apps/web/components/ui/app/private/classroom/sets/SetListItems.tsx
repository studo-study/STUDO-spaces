...
export default function SetListItems({ items }: ListItemProps) {
  ...
  return (
    <div
      className={`flex-1 overflow-y-scroll scroll-hidden h-full mb-10 z-10 flex flex-col gap-5 pb-15`}
    >
      {items.map((item, index) => (
        <ListItem set={item} key={index} index={index} t={t} locale={locale} />
      ))}
    </div>
  );
}
  ...
  return (
    ...
    >
      ...
      >
        <div
          className={
            "w-15 h-15 rounded-full bg-studogrey/30 flex items-center justify-center"
          }
        >
          <Image
            width={0}
        ...
        <div>
          ...
          <div
            className={
              "w-full dark:text-white/50 text-studodarkblue/50 text-sm flex items-center gap-1"
            }
          >
            <span>{t("added by")}</span>
            <Link href={"/profile" + set.user_id} className={"hover:underline"}>
              {set.displayName}
            </Link>
          </div>
        </div>
      ...
      <div
        className={`min-w-40 w-full sm:w-auto flex flex-row justify-between sm:gap-3 items-center`}
      >
        <div
          className={`w-full sm:w-auto flex flex-col sm:flex-row items-center gap-2 mr-2`}
        >
          {challenged && inChallenge(t)}
        </div>
        <span className={`text-white/30 text-sm min-w-20 max-w-20 text-center`}>
          {date}
        </span>
...
function inChallenge(t: ReturnType<typeof useTranslations>) {
  return (
    <div
      className={
        "w-fit px-5 pr-7 py-2 rounded-full bg-studoblue/30 hover:bg-studoblue/40 transition-all duration-300" +
