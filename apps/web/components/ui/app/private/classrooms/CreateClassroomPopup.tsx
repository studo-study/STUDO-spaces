...
export default function CreateClassroom({
  ...
  return (
    <div
      className={`fixed inset-0 flex items-baseline justify-center pt-70 w-full bg-black/50 h-full z-[9999] ${createOpen ? "opacity-100" : "pointer-events-none opacity-0"}`}
    >
      <div
        ref={popupRef}
      ...
      >
        ...
        <div className={"w-full h-fit flex flex-col gap-7"}>
          ...
          <div className={"w-full h-fit flex flex-col gap-3"}>
            <span className={"dark:text-white text-studodarkblue"}>
              {t("subtitle_title")}
            </span>
          ...
          <div className={"w-full h-fit flex flex-col gap-3"}>
            ...
            <div className="flex flex-col w-full gap-2 items-center justify-between">
              <select
                className={
                  "h-12 px-5 gap-5 text-white cursor-pointer w-full rounded-4xl glass-rgb transition-all duration-300 border appearance-none border-neutral-200/30 shadow-2xl focus:ring-0 outline-none flex justify-around"
                }
              >
