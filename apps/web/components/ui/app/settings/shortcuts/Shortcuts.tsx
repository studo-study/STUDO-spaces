...
function ShortcutItem({ shortcut, isMac }: { shortcut: Shortcut; isMac: boolean }) {
    const t = useTranslations("settings.shortcut_titles");

    const renderKey = (key: string) => {
        if (key === "command") return <KeyCap>{isMac ? <FiCommand /> : "ctrl"}</KeyCap>;
        if (key === "delete")  return <KeyCap><LuDelete size={14} /></KeyCap>;
...
function Select() {
    return (
        <Link href={"/select"} className={"px-3 rounded-3xl bg-studogrey/50"}>
            <span className={"font-bold h-fit bg-clip-text text-transparent truncate bg-linear-to-r from-indigo-300 to-blue-300"}>
                select
            </span>
        </Link>
    )
