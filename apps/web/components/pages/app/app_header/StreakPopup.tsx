...
export default function StreakPopup({Streak, StreakOpen, setStreakOpen, containerRef}: StreakPopupProps) {
    ...
    return (
      ...
      `}
        ...
        >
                <div className={`w-full h-full flex p-5 flex-col items-center justify-center
                    ...
                    <span className={`font-atrament font-sfpro font-bold text-xl text-orange-400
                    transition-all duration-200 ease-out
                    ${StreakOpen ? "opacity-100 translate-y-0" : "opacity-0 -translate-y-2"}`}
                    style={{transitionDelay: StreakOpen ? `${3 * 50}ms` : "0ms",}}>
                        {t('day streak')}
                    </span>
                    <span className={`font-sfpro text-xs text-gray-400 transition-all duration-200 ease-out
                        ${StreakOpen ? "opacity-100 translate-y-0" : "opacity-0 -translate-y-2"}`}
                          style={{transitionDelay: StreakOpen ? `${4 * 50}ms` : "0ms",}}>
                      {t('studied')}{Streak}{t('days')}.
                    </span>
                </div>
        </div>
    );
