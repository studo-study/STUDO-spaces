...
function mergeDirty(key: string, endpoint: string, method: string, payload: Record<string, unknown>) {
...
function syncBoardTotals(set: Function, get: Function) {
    ...
    const totalDone = rows.filter((r: FlowRowResponse) => r.status === 'done').length;
    const totalInProgress = rows.filter((r: FlowRowResponse) => r.status === 'doing').length;
    ...
    const updatedCourses = activeBoard.courses.map((c: FlowCourseResponse) =>
        c.id === activeCourse.id
            ? {...c, total_done: totalDone, total_in_progress: totalInProgress, total_length: totalLength}
            : c
    );

    // Recalc board totals
    const boardDone = updatedCourses.reduce((s: number, c: FlowCourseResponse) => s + c.total_done, 0);
    const boardInProgress = updatedCourses.reduce((s: number, c: FlowCourseResponse) => s + c.total_in_progress, 0);
    const boardLength = updatedCourses.reduce((s: number, c: FlowCourseResponse) => s + c.total_length, 0);
...
export const useFlowStore = create<FlowState>((set, get) => ({
    ...
    updateRow: (rowId, updates) => {
        ...
        if (rest.description !== undefined) payload.description = rest.description as string;
        if (rest.priority !== undefined) payload.priority = rest.priority as Priority;
        ...
        if (rest.studoset_id !== undefined) payload.studoset_id = rest.studoset_id as string;
        if (rest.visualset_id !== undefined) payload.visualset_id = rest.visualset_id as string;
        ...
        if (isTextOnly) {
            ...
            textTimers.set(rowId, setTimeout(() => {
                textTimers.delete(rowId);
                mergeDirty(dirtyKey, endpoint, 'PATCH', payload as Record<string, unknown>);
                scheduleDirtyFlush();
            }, 600));
        } else {
    ...
    // ─── UI actions ─────────────────────────────────────────
    toggleCreateCourse: () => set((s) => ({createCourseOpen: !s.createCourseOpen})),
