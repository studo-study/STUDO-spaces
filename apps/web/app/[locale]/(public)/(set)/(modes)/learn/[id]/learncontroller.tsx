...
type Phase = "answering" | "correct" | "incorrect" | "showAnswer" | "showProgress";
...
type Action =
    | { type: "SUBMIT_ANSWER"; input: string; correctAnswer: string; card: Card; cards: Card[] }
    | { type: "SHOW_ANSWER" }
...
function learnReducer(state: State, action: Action): State {
    switch (action.type) {
        case "SUBMIT_ANSWER": {
            ...
            const allMastered = isCorrect && action.cards.every(
                card => (newCounts[card.id] || 0) >= 2
            );
...
export default function LearnController({ data }: LearnProps) {
    ...
    const termTaal = data.global_term_language !== data.global_definition_language
        ? "term"
        : data.global_term_language;

    const defTaal = data.global_term_language !== data.global_definition_language
        ? "definitie"
