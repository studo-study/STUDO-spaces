...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
    ...
    const messages = (await import(`../../../../../../messages/seo/duel/${locale}.json`)).default;
                        ...
                        {launched &&
							<AnimateOnMount delay={1000} className={"w-full"}>
								<div className={`w-full flex items-center justify-baseline transition-all duration-700 delay-1000`}>
									<Link href="/register" className="px-6 py-3 rounded-full flex items-center justify-center text-white bg-cyan-400 font-bold">
                                        {t("cta")}
									</Link>
								</div>
							</AnimateOnMount>}
