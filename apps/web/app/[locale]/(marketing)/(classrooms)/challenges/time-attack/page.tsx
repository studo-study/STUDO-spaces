...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
    ...
    const messages = (await import(`../../../../../../messages/seo/time-attack/${locale}.json`)).default;
...
export default function SpeedRunPage() {
    const t = useTranslations("landing.challenges.speedrun")
    const launched = true;
    return (
        <main className={`w-full dark:text-white text-studodarkblue
          ...
          bg-gradient-to-b from-transparent via-transparent to-red-700/40`}>
                        ...
                        {launched &&
							<AnimateOnMount delay={1000} className={"w-full"}>
								<div className={`w-full flex items-center justify-baseline transition-all duration-700 delay-1000`}>
									<Link href="/register" className="px-6 py-3 rounded-full flex items-center justify-center text-white bg-red-400 font-bold">
                                        {t("cta")}
									</Link>
								</div>
							</AnimateOnMount>}
