...
type Phase = "input" | "feedback" | "intermezzo" | "done";
...
const LearnCard = () => {
  ...
  return (
    <div
      className={
        "w-200 h-150 bg-studogrey/30 border border-studoborder/30 rounded-4xl flex flex-col justify-between gap-3 p-3"
      }
    >
      <div
      ...
      >
        {phase === "intermezzo" ? (
          ...
          <ProgressBar cardLength={goal} cardIndex={totalViews} />
        )}
      </div>
      {phase === "intermezzo" ? (
        ...
        <>
          ...
          >
            ...
            <span className={"text-3xl font-semibold"}>{displayWord}</span>
            {phase === "feedback" && !wasCorrect && (
              <span
                className={classnames("text-xl font-medium", "text-rose-500")}
              >
                {corrector}
              </span>
            )}
