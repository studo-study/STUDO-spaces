...
export async function generateMetadata({
  ...
  return buildSeoMetadata("mastery", "/challenges/mastery-tournament", locale);
}

export default function MasteryPage() {
  const t = useTranslations("landing.challenges.mastery");
  const launched = true;
  return (
    <main
      className={`w-full dark:text-white text-studodarkblue
          ...
          bg-gradient-to-b from-transparent via-transparent to-orange-400/40`}
    >
            ...
            {launched && (
              <AnimateOnMount delay={1000} className={"w-full"}>
                ...
                >
                  <Link
                    href="/register"
                    className="px-6 py-3 rounded-full flex items-center justify-center text-white bg-orange-400 font-bold"
                  >
                    {t("cta")}
                  </Link>
                </div>
