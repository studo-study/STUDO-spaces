...
export default function Classroom() {
  ...
  const { trigger: triggerJoin } = useSWRMutation(
    `classrooms/${id}/users`,
    save,
  );
  const { trigger: triggerLeave } = useSWRMutation(
    `classrooms/${id}/users/me`,
    del,
  );
  ...
  return (
    <div className="w-full h-full flex flex-col gap-6 sm:gap-8 md:gap-10">
      {!isLoading && !joined && (
        <div className="w-full h-fit flex flex-col sm:flex-row gap-3 sm:gap-0 sm:justify-between sm:items-center">
          <span className="w-full text-xl sm:text-2xl font-bold break-words">
            {classroom.name}
          </span>
          <button
          ...
          >
            {joining
              ? t("joning...").toUpperCase()
              : t("join classroom").toUpperCase()}
          </button>
        </div>
      )}

      <div className="w-full h-full flex flex-col lg:flex-row gap-5">
        <div className="w-full lg:w-3/5 h-max flex flex-col gap-3 sm:gap-4 md:gap-5 justify-baseline items-baseline">
          ...
          {!isLoading && classroom?.sets?.length === 0 && (
            <span className="w-full h-fit flex pt-6 sm:pt-10 items-center justify-center text-sm sm:text-base">
        ...
        <div className="flex flex-col gap-6 sm:gap-8 md:gap-10 w-full lg:w-2/5">
          ...
          {!isLoading && joined && (
            <button
              disabled={classroom.users.length === 1}
            ...
            >
              {leaving
                ? t("leaving classroom...").toUpperCase()
                : t("leave classroom").toUpperCase()}
            </button>
          )}
        </div>
      </div>
