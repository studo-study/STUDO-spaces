...
export default function SearchResultItem({ item }) {
  ...
  return (
    <Link to={`/${item.type}/${item.id}`}>
      {item.type === "studyset"
        ? renderSet(hoverToggle, hovering, item)
        : item.type === "visualset"
          ? renderSet(hoverToggle, hovering, item)
          : item.type === "profile"
            ? profile(item)
...
function renderSet(hoverToggle, hovering, item) {
  return (
    <div
      className="w-full h-max rounded-2xl sm:rounded-3xl bg-studogrey flex flex-col p-3 sm:p-4
    ...
    >
      ...
      <div className="flex flex-row justify-baseline gap-1.5 sm:gap-2 items-center pb-6 sm:pb-10">
        <span className="w-fit px-2 py-0.5 sm:py-1 text-[10px] sm:text-xs font-bold bg-studogrey rounded-full sm:rounded-4xl">
...
function classroom(item) {
  return (
    <div
      className="w-full max-h-fit rounded-2xl sm:rounded-3xl bg-studogrey flex flex-col p-3 sm:p-4
      shadow-[8px_8px_16px_#bebebe,-8px_-8px_16px_#bebebe] backdrop-blur-xs
      dark:bg-gray-700 dark:shadow-[8px_8px_16px_#1a1a2a,-8px_-8px_16px_#1a1a2a]
      border-[0.5px] border-solid
      dark:border-t-gray-500 dark:border-l-gray-500
    ...
    >
      <div className="w-full h-fit flex flex-row items-center gap-3 sm:gap-5">
        <div className="w-10 h-10 sm:w-13 sm:h-13 rounded-full bg-studogrey overflow-hidden flex-shrink-0">
          <img
            src={item.studoProfile ? Studoicon : item.img_url}
            alt=""
            className="w-full h-full object-cover"
          />
        </div>
        <div className="flex flex-col items-baseline gap-1 sm:gap-2 min-w-0 flex-1">
          ...
          <span className="flex flex-row text-xs sm:text-sm items-center w-fit gap-1 px-2 sm:px-3 py-0.5 sm:py-1 rounded-full sm:rounded-3xl bg-studogrey">
            {item.role}
          </span>
        </div>
      </div>
