...
export default function Courses({ courses }) {
	return (
		<div className="relative w-full mt-2 sm:mt-3 md:mt-5">
			{courses.length === 0 ? (
				<div className="w-full h-28 sm:h-32 md:h-36 p-3 justify-center items-center flex text-sm sm:text-base
          ...
          border-[#8181812f] border-t-[#ffffff] border-l-[#f2f2f2]">
					<span className="text-studodarkblue/60 dark:text-white/60">{t("no courses yet")}</span>
				</div>
			) : (
          ...
          border-[0.5px] border-solid">
			...
			{/* Left gradient fade */}
			<div className="absolute left-0 top-0 bottom-0 w-3 sm:w-4 md:w-6
        bg-gradient-to-r from-bg-white dark:from-bg-dark to-transparent
        pointer-events-none rounded-l-2xl sm:rounded-l-3xl md:rounded-l-4xl"></div>

			{/* Right gradient fade */}
			<div className="absolute right-0 top-0 bottom-0 w-5 sm:w-8 md:w-12
        bg-gradient-to-l from-bg-white dark:from-bg-dark to-transparent
        pointer-events-none rounded-r-2xl sm:rounded-r-3xl md:rounded-r-4xl"></div>
		</div>
	);
}