      ...
      <div className="flex w-full sm:w-11/12 md:w-4/5 lg:w-3/5 flex-col items-center justify-center gap-3 sm:gap-4 md:gap-5">
        ...
        <div className="w-full h-fit flex flex-col gap-3 sm:gap-4 md:gap-5 mb-3 sm:mb-4 md:mb-5">
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col sm:flex-row gap-3 sm:gap-5 justify-between items-stretch">
              <Stats
                svg={Studyset}
                waarde={user.stats?.totalsets}
                title={t("Studysets")}
              />
              <Stats
                svg={Cards}
                waarde={user.stats?.cardsLearned}
                title={t("Cards Studied")}
              />
              <Stats
                svg={Time}
                waarde={user.stats?.timeLearned}
                title={t("Time Studied")}
              />
            </div>
          )}
        </div>
        ...
        <div className="w-full h-fit flex flex-col gap-3 sm:gap-4 md:gap-5">
          ...
          <div className="w-full h-10 flex flex-row justify-end items-center">
            <Link
              className="text-studodarkblue dark:text-white text-sm sm:text-base"
              to="/studysets"
            >
              {t("more sets")}
            </Link>
        ...
        <div className="w-full h-fit sm:h-20 flex flex-col sm:flex-row gap-3 sm:gap-0 sm:justify-between items-stretch sm:items-center mb-6">
        ...
        {deleteOpen ? (
          <DeleteAccount deleteOpen={deleteOpen} onClose={closeDelete} />
        ) : (
          ""
        )}
      </div>
