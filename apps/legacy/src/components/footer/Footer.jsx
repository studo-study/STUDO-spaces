...
export default function Footer() {
  ...
  return (
    <div
      className={"w-full  min-h-15 flex flex-row justify-center items-center"}
    >
      <div className="flex w-3/5 flex-row items-center justify-between gap-5">
        ...
        <div className={"flex flex-row gap-7"}>
          <Link to={"https://european-union.europa.eu/"}>
            {" "}
            <img src={made} alt="europe" className={"h-12 hidden"} />
          </Link>
          <select
          ...
          >
            {Object.entries(talen).map(([key, value]) => (
              <option
                selected={selLang === key ? true : false}
                key={key}
                value={key}
              >
                {t(value)}
