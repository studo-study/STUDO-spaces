...
export default function DeleteAccount({ deleteOpen, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-[999] flex items-center justify-center glass-rgb">
      <div
        className={"w-100 h-60 z-[9999] flex flex-col items-center\n         " +
        ...
        <div className={`flex flex-col items-baseline w-full h-full font-atrament text-[#2a3a42]
        ...
        bg-white" onClick={handleDelete}><img src={Delete} alt="delete-icon"
                                              className={"h-5"} />{t("Delete Account").toUpperCase()}</div>
          </div>
        </div>
