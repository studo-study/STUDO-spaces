...
export default function BestResult({ item }) {
  return (
    <Link to={`/${item.type}/${item.id}`}>
      {item.type === "studyset" ? renderSet(item)
        : item.type === "visualset" ? renderSet(item) :
...
function renderSet(item) {
  return (
    <div className="w-full sm:w-3/4 md:w-2/3 lg:w-1/2 h-max rounded-2xl sm:rounded-3xl
...
function classroom(item) {
  return (
    <div className="w-full sm:w-3/4 md:w-2/3 lg:w-1/2 max-h-fit rounded-2xl sm:rounded-3xl
      bg-studogrey flex flex-col p-3 sm:p-4
      shadow-[8px_8px_16px_#bebebe,-8px_-8px_16px_#bebebe] backdrop-blur-xs
      dark:bg-gray-700 dark:shadow-[8px_8px_16px_#1a1a2a,-8px_-8px_16px_#1a1a2a]
      border-[0.5px] border-solid
...
function profile(item) {
  return (
    <div className="w-full sm:w-3/4 md:w-1/2 lg:w-1/3 h-max rounded-2xl sm:rounded-3xl
      bg-studogrey flex flex-col p-3 sm:p-4
      shadow-[8px_8px_16px_#bebebe,-8px_-8px_16px_#bebebe] backdrop-blur-xs
      dark:bg-gray-700 dark:shadow-[8px_8px_16px_#1a1a2a,-8px_-8px_16px_#1a1a2a]
      border-[0.5px] border-solid
      ...
      <div className="w-full h-fit flex flex-row items-center gap-3 sm:gap-4">
        <div
          className="w-16 h-16 sm:w-20 sm:h-20 md:w-25 md:h-25 rounded-full overflow-hidden bg-amber-300 flex-shrink-0">
          <img src={item.verified ? Studoicon : item.img_url} alt="" className="w-full h-full object-cover" />
        </div>
        <div className="flex flex-col items-baseline gap-1 sm:gap-2 min-w-0 flex-1">
          <span className="text-lg sm:text-xl font-bold truncate w-full">{item.displayName}</span>
          <span
            className="bg-studogrey rounded-full sm:rounded-4xl px-2 sm:px-3 py-0.5 sm:py-1 text-xs sm:text-sm md:text-md flex items-center">
            {item.role}
          </span>
        </div>
      </div>
