  ...
  return (
    <div className="relative w-screen h-screen bg-white z-[99999]">
      <div className="absolute top-0   w-full h-22 flex flex-row justify-end items-center p-5">
        <button onClick={() => navigate(-1)}><img src={Cross} alt="sluit" className=" cursor-pointer h-8"/></button>
      </div>
      <div className="w-screen h-screen overflow-hidden flex flex-col gap-3 justify-center items-center">
        <div className=" h-[65] border-none flex justify-center items-center">
          <video autoPlay loop muted className="block w-90 h-90 backface-visibility-hidden border-none outline-none">
      ...
      border-gray-100 w-full h-22 flex flex-row justify-center items-center p-5">
        <div onClick={() => navigate(-1)}><div className="inline-flex flex-row gap-2 justify-center
        items-center p-2 pl-7 pr-7 bg-studoblue text-white font-atrament rounded-4xl cursor-pointer">{t('close').toUpperCase()}</div></div>
      </div>
    </div>);
}