...
export default function ProfileHeader({ user }) {
  ...
  return (
    <div className="flex flex-col sm:flex-row justify-start sm:justify-baseline items-start sm:items-center
      ...
      <div className="flex flex-col gap-2 sm:gap-3 w-full">
        <div className="flex flex-col sm:flex-row gap-2 sm:gap-0 sm:justify-baseline sm:items-center flex-wrap">
          <span className="flex items-center text-xl sm:text-2xl
                font-sfpro font-bold text-studodarkblue dark:text-white break-all">{user.displayName}</span>
          <Joined number={user.joinNumber} />
          {user.streak_count < 3 ? "" :
            <Link to="/streak"><Streak streak={user.streak_count} /></Link>}
        </div>
        <div className="flex flex-row justify-start items-center gap-2 sm:gap-3 flex-wrap">
          <img className="h-4 sm:h-5 dark:brightness-0 dark:invert" src={Cal} alt="calendar icon" />
