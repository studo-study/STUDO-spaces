...
export default function QuickStats({t}: QuickstatsProps) {
    ...
    return (<div className={"w-full grid grid-rows-1 grid-cols-4 gap-5"}>
        <div className={"w-full h-25 rounded-4xl dark:bg-studogrey/10 bg-studogrey border dark:border-studoborder/30 border-studodarkblue/20 p-5 flex flex-col gap-1"}>
            <span className={"w-full font-bold dark:text-white/50 text-studodarkblue/50 items-baseline flex gap-2 flex-row"}>{t("total_usr")} {perc(TotalthisMonth, TotallastMonth)}</span>
...
function checkTotalUsers(number: number, lastMonth: number, t: any) {
    const difference = number - lastMonth;
    const perc = Math.floor((difference / lastMonth) * 10000) / 100;
    if (difference >= lastMonth * 1.2 - lastMonth) return (<div className={"w-full flex flex-row items-center gap-2 text-2xl text-emerald-500"}><span className={"w-fit font-bold dark:text-white text-studodarkblue"}>{refactor(number)}</span><IoIosRocket size={20} /><span className={"text-sm dark:text-white/50 text-black/50"}>(+ {refactor(difference.valueOf())} )</span></div>);
    if (difference >= lastMonth * 1.1 - lastMonth) return (<div className={"w-full flex flex-row items-center gap-2 text-2xl text-emerald-500"}><span className={"w-fit font-bold dark:text-white text-studodarkblue"}>{refactor(number)}</span><BiSolidPlaneAlt size={20} /><span className={"text-sm dark:text-white/50 text-black/50"}>(+ {refactor(difference.valueOf())} )</span></div>);
    if (difference >= lastMonth * 1.05 - lastMonth) return (<div className={"w-full flex flex-row items-center gap-2 text-2xl text-emerald-300"}><span className={"w-fit font-bold dark:text-white text-studodarkblue"}>{refactor(number)}</span><IoTrendingUpOutline size={35} /><span className={"text-sm dark:text-white/50 text-black/50"}>(+ {refactor(difference.valueOf())} )</span></div>);
    if (difference > lastMonth - lastMonth) return (<div className={"w-full flex flex-row items-center gap-2 text-2xl dark:text-white text-studodarkblue"}><span className={"w-fit font-bold dark:text-white text-studodarkblue"}>{refactor(number)}</span><IoTrendingUpOutline size={35} /> <span className={"text-sm dark:text-white/50 text-black/50"}>(+ {refactor(difference.valueOf())} )</span></div>);
    if (difference === lastMonth - lastMonth) return (<div className={"w-full flex flex-row items-center gap-2 text-2xl dark:text-white text-studodarkblue"}><span className={"w-fit font-bold dark:text-white text-studodarkblue"}>{refactor(number)}</span><span className={"text-sm dark:text-white/50 text-black/50"}>(+ {difference})</span></div>);
    if (difference < 0) return (<div className={"w-full flex flex-row items-center just gap-2 text-2xl text-rose-500"}><span className={"w-fit font-bold dark:text-white text-studodarkblue"}>{refactor(number)}</span><IoIosTrendingDown size={35} /> <span className={"text-sm dark:text-white/50 text-black/50"}>(- {refactor(difference.valueOf())})</span></div>);
}

function checkActiveUsers(number: number, lastMonth: number, t: any) {
    const difference = number - lastMonth;
    const perc = Math.floor((difference / lastMonth) * 10000) / 100;
    if (difference >= lastMonth * 1.3 - lastMonth) return (<div className={"w-full flex flex-row items-center gap-2 text-2xl text-emerald-500"}><span className={"w-fit font-bold dark:text-white text-studodarkblue"}>{refactor(number)}</span><IoIosRocket size={20} /><span className={"text-sm dark:text-white/50 text-black/50"}>(+ {refactor(difference.valueOf())} )</span></div>);
    if (difference >= lastMonth * 1.25 - lastMonth) return (<div className={"w-full flex flex-row items-center gap-2 text-2xl text-emerald-500"}><span className={"w-fit font-bold dark:text-white text-studodarkblue"}>{refactor(number)}</span><BiSolidPlaneAlt size={20} /><span className={"text-sm dark:text-white/50 text-black/50"}>(+ {refactor(difference.valueOf())} )</span></div>);
    if (difference >= lastMonth * 1.1 - lastMonth) return (<div className={"w-full flex flex-row items-center gap-2 text-2xl text-emerald-300"}><span className={"w-fit font-bold dark:text-white text-studodarkblue"}>{refactor(number)}</span><IoTrendingUpOutline size={35} /><span className={"text-sm dark:text-white/50 text-black/50"}>(+ {refactor(difference.valueOf())} )</span></div>);
    if (difference > lastMonth - lastMonth) return (<div className={"w-full flex flex-row items-center gap-2 text-2xl dark:text-white text-studodarkblue"}><span className={"w-fit font-bold dark:text-white text-studodarkblue"}>{refactor(number)}</span><IoTrendingUpOutline size={35} /> <span className={"text-sm dark:text-white/50 text-black/50"}>(+ {refactor(difference.valueOf())} )</span></div>);
    if (difference === lastMonth - lastMonth) return (<div className={"w-full flex flex-row items-center gap-2 text-2xl dark:text-white text-studodarkblue"}><span className={"w-fit font-bold dark:text-white text-studodarkblue"}>{refactor(number)}</span><span className={"text-sm dark:text-white/50 text-black/50"}>(+ {difference})</span></div>);
    if (difference < 0) return (<div className={"w-full flex flex-row items-center just gap-2 text-2xl text-rose-500"}><span className={"w-fit font-bold dark:text-white text-studodarkblue"}>{refactor(number)}</span><IoIosTrendingDown size={35} /> <span className={"text-sm dark:text-white/50 text-black/50"}>(- {refactor(difference.valueOf())})</span></div>);
}
