        ...
        <div className={`h-full border-r select-none border-studoborder/30
            ...
            {main.map((item, index) => (
                <Link href={item.link} key={index} className={`w-full h-10 ${isActive(item.link) ? "opacity-75": "opacity-50"} px-5`}>
                    <div className={`transition-colors duration-200 flex gap-5 rounded-4xl hover:bg-studogrey ${isActive(item.link) ? "bg-studogrey" : ""} flex-row justify-baseline px-5 items-center w-full h-10`}>
                        <div className="flex items-center min-w-10 justify-center cursor-pointer text-2xl text-white">
                            {item.icon}
                        </div>
                        <span className={`text-white select-none whitespace-nowrap transition-[opacity,transform] duration-200
            ...
            <div className="w-full h-10 px-5">
                <div onClick={toggleCreate}
                     className="transition-colors duration-200 flex gap-5 select-none rounded-4xl opacity-50 cursor-pointer hover:bg-studogrey flex-row justify-baseline px-5 items-center w-full h-10">
                    <div className="flex items-center min-w-10 justify-center cursor-pointer text-2xl text-white">
                        <IoIosAdd />
                    </div>
                    <span className={`text-white select-none whitespace-nowrap transition-[opacity,transform] duration-200
            ...
            {classrooms.map((item, index) => (
                <Link href={item.link} key={index} className={`w-full h-10 ${isActive(item.link) ? "opacity-75": "opacity-50"} px-5`}>
                    <div className={`transition-colors duration-200 flex gap-5 select-none rounded-4xl hover:bg-studogrey ${isActive(item.link) ? "bg-studogrey" : ""} flex-row justify-baseline px-5 items-center w-full h-10`}>
                        <div className="flex items-center min-w-10 justify-center cursor-pointer text-2xl text-white">
                            {item.icon}
                        </div>
                        <span className={`text-white whitespace-nowrap select-none transition-[opacity,transform] duration-200
                            ${burgerOpen ? "opacity-100 translate-x-0 delay-100" : "opacity-0 -translate-x-4 pointer-events-none"}`}>
                            {t(item.label)}
                        </span>
                    </div>
            ...
            <div onClick={toggleSearch} className="w-full h-10 aria-selected:opacity-100 px-5 opacity-50 cursor-pointer">
                <div className="transition-colors duration-200 flex gap-5 select-none rounded-4xl hover:bg-studogrey flex-row justify-baseline px-5 items-center w-full h-10">
                    <div className="flex items-center min-w-10 justify-center cursor-pointer text-2xl text-white">
                        <IoSearch />
                    </div>
                    <span className={`text-white select-none whitespace-nowrap transition-[opacity,transform] duration-200
