...
export default function AddSet({addOpen, setAddOpen}: InvitePeopleProps) {
    ...
    const importedClassrooms: FullClassroomSet[] = [];
    useEffect(() => {
    ...
    return (
        <div className={`fixed inset-0 flex items-baseline justify-center pt-70 w-full bg-black/50 h-full z-[9999] ${addOpen ? "opacity-100" : "pointer-events-none opacity-0"}`}>
            ...
            >
                <div className={"w-full flex px-2 py-2 dark:text-white text-4xl text-studodarkblue items-center justify-end absolute top-0 left-0 z-10"}>
                    <div className={"h-8 w-8 rounded-full hover:bg-studogrey transition-all duration-300 justify-center items-center cursor-pointer active:scale-95 flex"}>
                        <IoIosClose  onClick={() => {setAddOpen(false)}}/>
                    </div>
                </div>
                <div className={"w-full h-fit flex flex-col gap-7"}>
                    ...
                    <div className={"w-full h-80 flex flex-col gap-3 overflow-y-auto"}>
                        {userSets.map((u, i) => <SetItem importedClassrooms={importedClassrooms} set={u} key={i}/>)}
                    </div>
