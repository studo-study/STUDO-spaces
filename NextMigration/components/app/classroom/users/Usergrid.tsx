...
export default function Usergrid(){
    ...
    <span className="font-bold text-lg dark:text-white text-studodarkblue">
        {t("users")}:
            ...
            <div className="w-full max-h-96 overflow-y-auto flex flex-col gap-5 scroll-hidden pb-20">
                {users.map((user, i) => (
                    <UserItem key={i} />
                ))}
            </div>
...
function UserItem() {
    return (<div  className={`max-h-22 min-h-22 flex-col sm:flex-row px-10 pl-4 py-4 sm:py-0 max-w-full min-w-full cursor-pointer
                 flex gap-3 shadow-2xl items-center w-full rounded-full bg-studogrey/10 border border-studogrey/20 
                 hover:border-studogrey/40 transition-all duration-300 overflow-hidden`}></div>)
}